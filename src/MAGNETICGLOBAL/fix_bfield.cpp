...
void FixBfield::post_force(int vflag)
{
  ...
  mui = 7812.5;
  bx = mui*cos(ntimestep*0.5*1.4e-4);
  by = mui*sin(ntimestep*0.5*1.4e-4);
  bz = 0;
  if (varflag == CONSTANT) {
    for (ii = 0; ii < inum; ii++) {
      i = ilist[ii];
      if ((mask[i] & groupbit) && atom->type[i]==2) {
        ...
        mum[i][0] = A*(3*mjr*dx-bx);
        mum[i][1] = A*(3*mjr*dy-by);
        mum[i][2] = A*(3*mjr*dz-bz);
        ...
        for (jj = 0; jj<jnum; jj++)  {
          ...
          mum[i][0] += A*(3*mjr*dx-mum[j][0]);
          mum[i][1] += A*(3*mjr*dy-mum[j][1]);
          mum[i][2] += A*(3*mjr*dz-mum[j][2]);

          //std::cout<<i<<j<<"mu = "<<mu[i][0]<<","<<mu[i][1]<<","<<mu[i][ 2]<<"\n";
        }

        mumu = mum[i][0]*mum[i][0]+mum[i][1]*mum[i][1]+mum[i][2]*mum[i][2];
        mum[i][3] = sqrt(mumu);
        if (mumu > C*C*4/u/u){
    ...
    if (mumflag) {
      for (ii = 0; ii < inum; ii++) {
        ...
        if ((mask[i] & groupbit) && atom->type[i]==2) {
          ...
          dy /= r;
          dz /= r;
          mjr = mum[1][0]*dx+mum[1][1]*dy+mum[1][2]*dz;
          mum[i][0] = A*(3*mjr*dx-mum[j][0]);
          mum[i][1] = A*(3*mjr*dy-mum[j][1]);
          mum[i][2] = A*(3*mjr*dz-mum[j][2]);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (mask[i] & groupbit) {
      ...
      mjr = bx*dx+by*dy+bz*dz;
      mumu = mum[i][0]*bx+mum[i][1]*by+mum[i][2]*bz;
      //std::cout<<"fm = "<<fx<<","<<fy<<","<<fz<<" ";

      f[i][0] += K*(mir*bx+mjr*mum[i][0]+(mumu-5*mjr*mir)*dx);
      f[i][1] += K*(mir*by+mjr*mum[i][1]+(mumu-5*mjr*mir)*dy);
      f[i][2] += K*(mir*bz+mjr*mum[i][2]+(mumu-5*mjr*mir)*dz);
